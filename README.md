...
Computer time counts only while go/outside is running. It uses the hardware-input idle timer and stops after 60 seconds without keyboard or mouse input, during sleep or display-off, and while the session is inactive. It does not import historical Screen Time. The menu bar shows only the ratio circle, with durations in its tooltip and popover. The circle compares the two durations: solid is computer time divided by computer time plus daylight remaining; hollow is daylight remaining. The popover clocks show hours, minutes, and seconds (`1h 34m 12s`, omitting zero-valued units), with tall condensed numerals, smaller muted unit letters, and labels underneath.

The main popover has a 120-point banner above two equal clock panels, with no footer or motivational copy. The banner contains only colorful abstract light artwork: an airy, vibrant two-hue gradient with gentle distortion and a broad bright area. The bright area travels from left to right between sunrise and sunset, bending the surrounding color field; brightness, palette, and shape follow the day's light, with subtle variation by date. After sunset and before sunrise, the banner uses deep indigo and a restrained cool glow. The right clock changes from Daylight left to Until sunrise, counting down to the next calculated sunrise, then switches back at sunrise. The comparison below uses evenly spaced black-and-white text over a restrained native glass blur. Both appearances and Reduce Transparency are supported; the gradient is drawn locally without images or an extra service.
